...
# Build solution
dotnet build asteroids.sln --configuration Release

# Run all tests
dotnet test asteroids.sln

# Run a single test
dotnet test AsteroidsWeb.Tests --filter "FullyQualifiedName~SessionServiceTests.CreateSession_ShouldCreateSessionWithFruitName"
...
### Overview
A multiplayer Asteroids game with real-time synchronization via SignalR.

- **Frontend**: Single HTML5 Canvas file (`AsteroidsWeb/wwwroot/index.html`) with embedded CSS/JS - no build step
