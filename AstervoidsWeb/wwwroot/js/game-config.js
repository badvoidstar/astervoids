...
const AstervoidsConfig = (function() {
    ...
    const CONFIG_CONTROLS = Object.freeze([
        ...
        control({
            ...
            help: 'Horizontal dead-zone half-extent for rectilinear turning.',
        }),
        control({
            ...
            min: 0, max: 60, step: 1,
            fmt: value => `${Math.round(value)} px`,
            help: 'Vertical dead-zone half-extent for rectilinear thrust and braking.',
        }),
        control({
            key: 'TOUCH_POLAR_DEADZONE_PX',
            label: 'Polar left anchor dead-zone radius (CSS px)',
            min: 0, max: 80, step: 1,
            fmt: value => `${Math.round(value)} px`,
            help: 'Minimum polar-anchor radius before turn and thrust input begins.',
        }),
        control({
            key: 'TOUCH_POLAR_THRESHOLD_PX',
            label: 'Polar left anchor threshold radius (CSS px)',
