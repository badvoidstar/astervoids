# Astervoids Game Implementation Plan (Snapshot: January 20, 2026)
...
- **Dynamic Background Beat**: Two-tone heartbeat that speeds up as astervoids are destroyed (like original arcade)
...
More astervoids → slower beat (1000ms)
Fewer astervoids → faster beat (150ms)
```
...
| **Astervoids** | ASTEROID_BASE_COUNT | 1 | Starting astervoids (wave 1) |
...
| | WAVE_ASTEROID_INCREMENT | 1 | Additional astervoids per wave |
...
### Safe Spawn Distance
New astervoids spawn at safe distance from ship:
```javascript
